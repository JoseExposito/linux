...
static const struct apple_key_translation apple2015_fn_keys[] = {
		...
		struct hid_usage *usage, __s32 value)
{
	...
	if (fnmode) {
		if (hid->product == USB_DEVICE_ID_APPLE_MAGIC_KEYBOARD_2015 ||
		    hid->product == USB_DEVICE_ID_APPLE_MAGIC_KEYBOARD_NUMPAD_2015)
			table = apple2015_fn_keys;
...
static void apple_setup_input(struct input_dev *input)
{
	...
	for (trans = apple2015_fn_keys; trans->from; trans++)
		set_bit(trans->to, input->keybit);
