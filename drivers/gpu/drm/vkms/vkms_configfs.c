					      ...
					      const char *name)
{
	...
	plane_cfg = vkms_config_add_plane(dev->config);
	if (IS_ERR(plane_cfg)) {
	...
	crtc_cfg = vkms_config_add_crtc(dev->config);
	if (IS_ERR(crtc_cfg)) {
		ret = PTR_ERR(crtc_cfg);
		goto free_config;
	}

	encoder_cfg = vkms_config_add_encoder(dev->config);
	if (IS_ERR(encoder_cfg)) {
		ret = PTR_ERR(encoder_cfg);
		goto free_config;
	}

	connector_cfg = vkms_config_add_connector(dev->config);
