...
#define plane_possible_crtcs_item_to_vkms_configfs_plane(item) \
	container_of(to_config_group((item)), struct vkms_configfs_plane, possible_crtcs_group)
...
#define encoder_possible_crtcs_item_to_vkms_configfs_encoder(item) \
	container_of(to_config_group((item)), struct vkms_configfs_encoder, possible_crtcs_group)
...
#define connector_possible_encoders_item_to_vkms_configfs_connector(item) \
	container_of(to_config_group((item)), struct vkms_configfs_connector, possible_encoders_group)
...
static ssize_t crtc_writeback_show(struct config_item *item, char *page)
{
	...
	writeback = crtc->config->writeback;
				    ...
				    size_t count)
{
	...
	crtc->config->writeback = writeback;
					    ...
					    const char *name)
{
	...
	crtc->config = vkms_config_add_crtc(dev->config);
					     ...
					     const char *name)
{
	...
	plane->config = vkms_config_add_plane(dev->config);
					       ...
					       const char *name)
{
	...
	encoder->config = vkms_config_add_encoder(dev->config);
						 ...
						 const char *name)
{
	...
	connector->config = vkms_config_add_connector(dev->config);
